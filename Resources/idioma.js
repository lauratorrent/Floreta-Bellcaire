...
alphabet.de_DE.Mix = {
    "photos" : [
        ...
        './images/de_pupurri_lletres/quark.jpg',
    ...
    "reverseCard" : [
        ...
        './images/de_pupurri_html/quark.html',
...
//main view in the window
var mainView = Ti.UI.createView({
	backgroundColor:'black'
});

idiomaWin.add(mainView);

/////////////////////// Functions definitions///////////////////////////////////
//function to create a view
function createNewView (index){
    var newView = Ti.UI.createImageView({        
        backgroundImage : alphabet[idioma][buttonType].photos[index]
    });
    return newView;
}

//function to create a reverse
...
//initial images
scrollView.addView(createNewView(0));
scrollView.addView(createNewView(1));
scrollView.addView(createNewView(2));

mainView.add(scrollView);
var lengthAbc = alphabet[idioma][buttonType].photos.length;
var previousPage = 0;
var positionAbc = 0;
var currentPage;
...
scrollView.addEventListener('scroll', function(e)
{
  ...
  //direction definition  
	...
	{
    e.direction = "right";	
   Ti.API.info('position  ' +e.direction);
	} 
	...
	Ti.API.info('scroll 1. positionAbc ' + positionAbc+ ' direction ' +direction+ ' previousPage ' +previousPage+ ' currentPage ' +currentPage);
  ...
  if (e.direction === 'left') {
    ...
    else if(positionAbc === 1)
    {
      positionAbc--;
      previousPage = currentPage + 1;
    }
    else if (positionAbc > 1 && positionAbc < lengthAbc - 1) 
    {
      //Ti.API.info('1. positionAbc ' +positionAbc);
      positionAbc--;
      //Ti.API.info('2. positionAbc ' +positionAbc);
      previousPage = currentPage + 1;     

      scrollView.views = [];     
      view0 = createNewView(positionAbc - 1);
      view1 = createNewView(positionAbc);
      view2 = createNewView(positionAbc + 1);       
      scrollView.views = [view0, view1, view2]; 
      
      scrollView.currentPage = 1;
    }
    else if(positionAbc === lengthAbc - 1)
    {
      positionAbc--;
      previousPage = currentPage;
    }   
    ...
    if(positionAbc === 0) // || positionAbc === 1)
    {
      positionAbc++;
      previousPage = currentPage -1;	      
      scrollView.currentPage = 1;
    ...
    else if (positionAbc > 0 && positionAbc < lengthAbc - 2) 
    {
     ...
     previousPage      
      ...
      //create the new scrollView.views
      scrollView.views = [];
      view0 = createNewView(positionAbc - 1);
      view1 = createNewView(positionAbc);
      view2 = createNewView(positionAbc + 1);
      ...
      Ti.API.info('scroll 2. positionAbc ' + positionAbc+ ' previousPage ' +previousPage+ ' currentPage ' +currentPage);
      ...
      //el el bo// scrollView.currentPage = 1;  	
      
      /*
      var player = Ti.Media.createSound({url:"./sounds/sound.wav"});
      player.play();
      */
      
      //Titanium.UI.ScrollableView.scrollToView(scrollView.views[1]);
      
    }
    else if(positionAbc === lengthAbc - 2)
    {
        positionAbc++;
        previousPage = currentPage;	
    }      
...
{   
  var webViewCard = getWebViewCard(alphabet[idioma][buttonType].reverseCard[positionAbc]);
          	
  //Ti.API.info('scroll positionAbc ' + positionAbc);
...
//create buttonAbc
var buttonAbc = Titanium.UI.createButton({
...
//click buttonAbc
buttonAbc.addEventListener('click', function(e) {   
 
  var abcWindow = Titanium.UI.createWindow({
      backgroundColor : 'black',
  ...
  var abcScrollView = Titanium.UI.createScrollView({
    	contentWidth:'auto',
    	contentHeight:'auto',
    	top:0,
...
*/
  ...
  var indexAbc=0;  
  for( var i = 0; i < (lengthAbc/4); i++ )
  {
    for( var j = 0; j < 4; j++ ){
      if (indexAbc < lengthAbc)
      {
        var x = j*70 + 8*(j+1);
        var y = i*105 + 10*(i+1);
        
        var miniButton = Titanium.UI.createButton({
          backgroundImage: alphabet[idioma][buttonType].photos[indexAbc],
  ...
  abcScrollView.add(view);
    
  abcWindow.add(abcScrollView);  

  //abcWindow.open({transition:Ti.UI.iPhone.AnimationStyle.CURL_DOWN});
  abcWindow.open();
  
   miniButton.addEventListener('click', function(e) { 
    //abcWindow.close({transition:Ti.UI.iPhone.AnimationStyle.CURL_UP});     
    abcWindow.close();
  });
